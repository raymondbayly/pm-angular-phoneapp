(function (global, factory) {
    ...
    (factory((global.od = global.od || {}, global.od.virtualscroll = {}),global.ng.core,global.rxjs,global.rxjs.operators,global.ng.common));
    ...
    var VirtualRowComponent = /** @class */ (function () {
        ...
        VirtualRowComponent.propDecorators = {
            _viewContainer: [{ type: core.ViewChild, args: ['viewRef', { read: core.ViewContainerRef },] }],
            getTransform: [{ type: core.HostBinding, args: ['style.transform',] }]
        };
     ...
     * @return {?}
     */
    function calcMeasure(rect, options) {
    ...
    var VirtualScrollComponent = /** @class */ (function () {
        function VirtualScrollComponent(_elem, _cdr, _componentFactoryResolver, _obsService, _zone) {
            ...
            this._zone = _zone;
            this.vsData = rxjs.empty();
            this.vsOptions = rxjs.empty();
            this.vsResize = rxjs.empty();
            this.vsUserCmd = rxjs.empty();
        ...
        function () {
            ...
            var /** @type {?} */ initData = [];
            var /** @type {?} */ data$ = /** @type {?} */ (this.vsData.pipe(operators.startWith(initData), operators.publish()));
            var /** @type {?} */ dataMeta$ = data$.pipe(operators.map(function (data) { return [(new Date()).getTime(), data.length]; }));
            var /** @type {?} */ defaultOptions = { itemWidth: 100, itemHeight: 100, numAdditionalRows: 1 };
            var /** @type {?} */ options$ = /** @type {?} */ (this.vsOptions.pipe(operators.startWith(defaultOptions), operators.publish()));
            var /** @type {?} */ rect$ = rxjs.merge(rxjs.fromEvent(window, 'resize'), this.vsResize).pipe(operators.debounceTime(this.vsDebounceTime, rxjs.animationFrameScheduler), operators.map(function () { return getContainerRect(); }), operators.startWith(getContainerRect()), operators.map(function (_a) {
                var width = _a.width, height = _a.height;
                return ({ width: width, height: height });
            }));
            var /** @type {?} */ scroll$ = new rxjs.Subject();
            this._zone.runOutsideAngular(function () {
                _this._subs.push(rxjs.fromEvent(_this._elem.nativeElement, 'scroll')
                    .pipe(operators.debounceTime(_this.vsDebounceTime, rxjs.animationFrameScheduler))
                    .subscribe(function () {
                    _this._zone.run(function () { return scroll$.next(); });
                }));
            });
            var /** @type {?} */ scrollTop$ = scroll$.pipe(operators.map(function () { return getScrollTop(); }), operators.startWith(0));
            var /** @type {?} */ measure$ = /** @type {?} */ (rxjs.combineLatest(rect$, options$).pipe(operators.map(function (_a) {
                var rect = _a[0], options = _a[1];
                return calcMeasure(rect, options);
            }), operators.publish()));
            var /** @type {?} */ scrollWin$ = /** @type {?} */ (rxjs.combineLatest(scrollTop$, measure$, dataMeta$, options$).pipe(operators.map(function (_a) {
            ...
            }), operators.distinctUntilChanged(function (prevWin, curWin) {
            ...
            var /** @type {?} */ dScrollWin$ = scrollWin$.pipe(operators.pairwise());
            var /** @type {?} */ renderCmd$ = /** @type {?} */ (dScrollWin$.pipe(operators.concatMap(function (_a) {
                var prevWin = _a[0], curWin = _a[1];
                var /** @type {?} */ rowsDiffCmd$ = rxjs.of(new NoopCmd());
                var /** @type {?} */ rowsUpdateCmd$ = rxjs.of(new NoopCmd());
                ...
                if (!isEmpty(removeRowsMap)) {
                    ...
                    rowsDiffCmd$ = rxjs.concat(rxjs.from(removeItemCmds_1.reverse()), rxjs.from(removeRowCmds));
                }
                    ...
                    rowsDiffCmd$ = rxjs.concat(rxjs.from(createRowCmds), rxjs.from(createItemCmds_1));
                }
                ...
                if (!isEmpty(existingRows)) {
                    ...
                    rowsUpdateCmd$ = rxjs.concat(rxjs.merge(rxjs.from(removeItemCmds_2.reverse()), rxjs.from(createItemCmds_2), rxjs.from(updateItemCmds_1), rxjs.from(shiftRowCmds)), rxjs.merge(rxjs.from(columnDiffRemoveItemCmds_1.reverse()), rxjs.from(columnDiffCreateItemCmds_1)));
                }
                return rxjs.merge(rowsDiffCmd$, rowsUpdateCmd$);
            }), operators.publish()));
            var /** @type {?} */ updateScrollWinFunc$ = scrollWin$.pipe(operators.map(function (scrollWindow) { return function (state) {
                state.scrollWindow = scrollWindow;
                _this._obsService.emitScrollWin([scrollWindow]);
                state.needsCheck = true;
                return state;
            }; }));
            var /** @type {?} */ createRowFunc$ = renderCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === CmdOption.CreateRow; }), operators.map(function (cmd) { return function (state) {
            ...
            var /** @type {?} */ removeRowFunc$ = renderCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === CmdOption.RemoveRow; }), operators.map(function (cmd) { return function (state) {
            ...
            var /** @type {?} */ shiftRowFunc$ = renderCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === CmdOption.ShiftRow; }), operators.map(function (cmd) { return function (state) {
            ...
            var /** @type {?} */ createItemFunc$ = renderCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === CmdOption.CreateItem; }), operators.withLatestFrom(data$), operators.map(function (_a) {
            ...
            var /** @type {?} */ updateItemFunc$ = renderCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === CmdOption.UpdateItem; }), operators.withLatestFrom(data$), operators.map(function (_a) {
                var cmd = _a[0], data = _a[1];
                return function (state) {
            ...
            var /** @type {?} */ removeItemFunc$ = renderCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === CmdOption.RemoveItem; }), operators.map(function (cmd) { return function (state) {
            ...
            var /** @type {?} */ userCmd$ = /** @type {?} */ (this.vsUserCmd.pipe(operators.publish()));
            var /** @type {?} */ userSetScrollTop$ = userCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === UserCmdOption.SetScrollTop; }));
            var /** @type {?} */ focusRowSetScrollTop$ = userCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === UserCmdOption.FocusRow; }), operators.withLatestFrom(scrollWin$), operators.map(function (_a) {
                var cmd = _a[0], scrollWin = _a[1];
                var /** @type {?} */ focusRow = /** @type {?} */ (cmd);
                return new SetScrollTopCmd(focusRow.rowIndex * scrollWin.itemHeight);
            }));
            var /** @type {?} */ focusItemSetScrollTop$ = userCmd$.pipe(operators.filter(function (cmd) { return cmd.cmdType === UserCmdOption.FocusItem; }), operators.withLatestFrom(scrollWin$), operators.map(function (_a) {
                var cmd = _a[0], scrollWin = _a[1];
            ...
            var /** @type {?} */ setScrollTopFunc$ = rxjs.merge(userSetScrollTop$, focusRowSetScrollTop$, focusItemSetScrollTop$)
                .pipe(operators.map(function (cmd) { return function (state) {
            ...
            // Update store
            var /** @type {?} */ main$ = rxjs.merge(createRowFunc$, removeRowFunc$, shiftRowFunc$, createItemFunc$, removeItemFunc$, updateItemFunc$, updateScrollWinFunc$, setScrollTopFunc$)
                .pipe(operators.scan(scanFunc, { measurement: null, scrollWindow: null, rows: {}, needsCheck: false }));
            this._subs.push(main$
                .pipe(operators.filter(function (state) { return state.needsCheck && state.scrollWindow !== null; }))
        ...
        VirtualScrollComponent.propDecorators = {
            ...
            _viewContainer: [{ type: core.ViewChild, args: ['viewRef', { read: core.ViewContainerRef },] }],
            vsData: [{ type: core.Input }],
            vsOptions: [{ type: core.Input }],
            vsResize: [{ type: core.Input }],
            vsUserCmd: [{ type: core.Input }],
            vsDebounceTime: [{ type: core.Input }],
            vsEqualsFunc: [{ type: core.Input }]
        };
